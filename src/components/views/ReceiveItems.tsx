...
//                     }
//                     if (data.transportingAmount === undefined) {
//                         ctx.addIssue({ path: ['transportingAmount'], code: z.ZodIssueCode.custom });
//                     }
//                 }
//             }
//         });
...
//             // Create the row object with exact field names matching ReceivedSheet type
//             const row: Partial<ReceivedSheet> = {
//                 timestamp: new Date().toISOString(),
//                 indentNumber: selectedIndent?.indentNumber || '',
//                 poDate: selectedIndent?.poDate || '',
...
//             // Handle photo of product upload
//             if (values.photoOfProduct !== undefined) {
...
//                     import.meta.env.VITE_PRODUCT_PHOTO_FOLDER
//                 );
//                 console.log('Product photo uploaded:', row.photoOfProduct);
//             }

//             // Handle photo of bill upload
//             if (values.photoOfBill !== undefined) {
...
//                     import.meta.env.VITE_BILL_PHOTO_FOLDER
//                 );
//                 console.log('Bill photo uploaded:', row.photoOfBill);
//             }
            
//             console.log('Submitting to RECEIVED sheet:', row);
            
//             // Insert into RECEIVED sheet
//             await postToSheet([row], 'insert', 'RECEIVED');

//             console.log('Updating indent sheet...');
            
//             // Update indent sheet to mark as received
//             await postToSheet(
//                 indentSheet
//                     .filter((s) => s.indentNumber === selectedIndent?.indentNumber)
//                     .map((prev) => ({
...
//             toast.success(`Item received successfully for ${selectedIndent?.indentNumber}`);
//             setOpenDialog(false);
//             setTimeout(() => updateAll(), 1000);
//         } catch (error) {
//             console.error('Error in form submission:', error);
//             toast.error('Failed to process received item. Please try again.');
//         } finally {
//             setLoading(false);
//         }
//     }
...
export default () => {
    ...
    async function onSubmit(values: z.infer<typeof schema>) {
        try {
            ...
            // DO NOT update indent sheet - let formulas handle actual5
            // Removed the indent sheet update to allow formulas to work
            ...
            toast.error('Failed to process received item');
        } finally {
            setLoading(false);
        }
    }
